...
class BasketViewTest(APITestCase):
    def setUp(self):
        ...
        self.image = Image.objects.create(src='images/test.png', alt='картинка')
        self.category = Category.objects.create(title='Мониторы', image=self.image)
    ...
    def test_add_item_to_basket(self):
        ...
        """
        response = self.client.post('/api/basket', {'id': self.product.id, 'count': 2}, format='json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_item_count(self):
        ...
        """
        response = self.client.post('/api/basket', {'id': self.product.id, 'count': 5}, format='json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_item_from_basket(self):
        ...
        """
        self.client.post('/api/basket', {'id': self.product.id, 'count': 2}, format='json')
        response = self.client.delete('/api/basket', {'id': self.product.id, 'count': 2}, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, [])
    ...
    def test_partial_delete_item(self):
        ...
        """
        self.client.post('/api/basket', {'id': self.product.id, 'count': 5}, format='json')
        response = self.client.delete('/api/basket', {'id': self.product.id, 'count': 3}, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data[0]['count'], 2)
...
class OrdersViewTest(APITestCase):
    ...
    def test_create_order(self):
        ...
        response = self.client.post('/api/orders', basket, format='json')
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(order.totalCost, self.product1.price * 2 + self.product2.price * 1 + 200)
    ...
    def test_get_orders_after_create(self):
        ...
        self.client.post('/api/orders', basket, format='json')
        response = self.client.get('/api/orders')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 1)
...
class OrderDetailViewTest(APITestCase):
    ...
    def test_get_order_detail(self):
        ...
        self.assertEqual(response.data['products'][0]['title'], self.product.title)

    def test_confirm_order(self):
        # Получаю из БД:
        response = self.client.get(f'/api/order/{self.order.id}')
        print(f"DELIVERY_TYPE: {response.data['deliveryType']}")

        response = self.client.post(f'/api/order/{self.order.id}')
        self.assertEqual(response.status_code, 200)
        self.order.refresh_from_db()
        self.product.refresh_from_db()
        print(f"DELIVERY_TYPE: {self.order.deliveryType}")
...
class PaymentViewTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.image = Image.objects.create(src='images/test.png', alt='Test Image')
        self.category = Category.objects.create(title='Мониторы', image=self.image)
        self.product = Product.objects.create(
            category=self.category,
            title='Монитор',
        ...
        self.product.images.add(self.image)
        self.basket_item = BasketItem.objects.create(user=self.user, product=self.product, count=2)
        self.order = Order.objects.create(
            user=self.user,
            fullName=self.user.fullName,
            email=self.user.email,
    ...
    def test_payment_success(self):
        ...
        """
        response = self.client.post(f'/api/payment/{self.order.id}', self.payment_data, format='json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_payment_order_not_found(self):
        ...
        Тестирование получения заказа, которого не существует.
        """
        response = self.client.post('/api/payment/9999999', self.payment_data, format='json')
        self.assertEqual(response.status_code, 404)
        self.assertIn('error', response.data)
